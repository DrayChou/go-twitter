...
func respond(w http.ResponseWriter, t string, data interface{}) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")

	tmpl := template.Must(template.ParseFiles("template/" + t + ".tmpl"))
...
func serveMessages(w http.ResponseWriter, r *http.Request, cred *oauth.Credentials) {
	...
	if err := apiGet(
		cred,
		"http://api.twitter.com/1.1/direct_messages.json",
		nil,
		&dms); err != nil {
		http.Error(w, "Error getting timeline, "+err.Error(), 500)
		return
	}
	respond(w, "messages", dms)
}
